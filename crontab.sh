# Sports ETL — replaces Prefect scheduled deployments (times are local)
# Install: crontab -e
SHELL=/bin/bash
...
# statcast-update-recent (10:00) — ingest + statcast_extra
...
# mlbam-pbp-update-recent (10:30) — schedule, recent PBP, then dbt season stats (inside update-recent)
30 10 * * * cd "$ETL_ROOT" && flock -n /tmp/mlbam-pbp-update-recent.lock bash -lc '"$UV" run --extra dbt python update_mlb_schedule.py && "$UV" run --extra dbt python update_mlbam_pbp.py update-recent --days 3' >> "$ETL_LOG/mlbam-pbp-update-recent.log" 2>&1
