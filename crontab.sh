...
# mlbam-pbp-update-recent (10:30 UTC) — schedule, recent PBP, then dbt season stats (inside update-recent)
30 10 * * * cd "$ETL_ROOT" && flock -n /tmp/mlbam-pbp-update-recent.lock bash -lc '"$UV" run --extra dbt python update_mlb_schedule.py season && "$UV" run --extra dbt python update_mlbam_pbp.py update-recent --days 3' >> "$ETL_LOG/mlbam-pbp-update-recent.log" 2>&1
